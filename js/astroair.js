...
*/
		...
		function connect(){		//连接websocket
			...
			inputField.value=urlpart,
			tcpPort=getTcpPort();
			let ws_protocol=getWs_protocol();		//ws为不加密
			...
			//"localhost"===urlpart?(wsUri=ws_protocol+urlpart.toString()+"/wss",createWebSocket()):""===urlpart?(wsUri=ws_protocol+"localhost/wss",createWebSocket()):(wsUri=ws_protocol+urlpart.toString()+"/wss",createWebSocket()),saveLocalStorage(inputField.value)
		}
		...
		function logEventReceived(msg){		//接收日志事件
			...
			writeToLog(msg.TimeInfo+" &emsp;<span class='logMsgType "+logColors[type]+"'>[ "+logType[type]+" ]</span> &emsp; "+text),
			(5!==type&&6!==type)||($("#generalActionText").text(text),showStatusBar(msg.Text))
		}
		...
		function newJPGReadyReceived(msg){		//新的JPEG已经生成，接收对应的图像信息
			...
			var imgObj={};
			""===msg.SequenceTarget?(
				$("#SequenceTarget").text("No Target"),
				imgObj.Target="No Target"
			):(
				$("#SequenceTarget").text(msg.SequenceTarget),
				imgObj.Target=msg.SequenceTarget
			),		//判断是否为计划拍摄，并显示
		...
		function RemoteCameraShot(){		//相机拍摄
			...
			Req.params.UID=generateUID(),
			Req.params.Expo=Number($("#ex").val()),		//曝光时间
			Req.params.Bin=Number($("#bin").val());		//像素合并
			Req.params.IsROI=!1,
			...
			Req.params.Gain=Number($("#gain").val()),		//增益
			Req.params.Offset=Number($("#offset").val()),		//偏执
			Req.id=generateID(),
		...
		function remoteCooling(obj){		//将制冷信息发送至服务端
			...
			Req.params.Temperature=Number($("#cameraTemp").val()),
			Req.id=generateID(),
		...
		remoteAstroAirProfileStartupReceived=msg=>{
			let temparr,activeProfile=msg.name;
			...
			startupConnected||(
				...
				temparr.length>0?(
					temparr.forEach(profItem=>{
						...
						contPh="active",
						placeholder=profItem.isactive?contPh:"选择";
